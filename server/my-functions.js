...
/**
 * Returns specified date as string in specified format
 * @param date
 * @param format
 * @param utc
 * @returns {string | *}
 */
exports.formatDate = function (date, format, utc) {
...
/**
 * Returns current time as string in DATETIME format
 * @returns {string|*}
 */
exports.getCurrentTime = function () {
    return this.formatDate(new Date(), consts.MYSQL_DATETIME_FORMAT);
};


/**
 * Checks response of the server for errors and send the data with status code
 * @param req
 * @param res
 * @param err
 * @param data
 * @param noRoute
 */
exports.apiGetResponseCheck = function (req, res, err, data, noRoute=false) {
    ...
    if (err) {
        ...
        if (data.length === 0) {
            ...
            // BAD REQUEST
            if (noRoute) {
                status = consts.HTTP_STATUS_400;
                res.status(400).send(status);
            }

            // NO CONTENT IN DB
            else {
                status = consts.HTTP_STATUS_204;
